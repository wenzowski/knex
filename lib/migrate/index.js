...
var Migrator = function () {
  ...
  Migrator.prototype.latest = function latest(config) {
    ...
    return this._migrationData(this.config.trx).tap(validateMigrationList).spread(function (all, completed) {
      ...
      if (_this.config.trx) {
        return _this._runBatch(migrations, 'up', _this.config.trx);
      }
  ...
  Migrator.prototype.rollback = function rollback(config) {
    ...
    return _bluebird2.default.try(function () {
      ...
      return _this2._migrationData(_this2.config.trx).tap(validateMigrationList).then(function (val) {
        return _this2._getLastBatch(val);
      }).then(function (_migrations) {
        var migrations = (0, _map3.default)(_migrations, 'name');

        var transactionForAll = !_this2.config.disableTransactions && (0, _isEmpty3.default)((0, _filter3.default)(migrations, function (name) {
          var migration = require(_path2.default.join(_this2._absoluteConfigDir(), name));
          return !_this2._useTransaction(migration);
        }));

        if (_this2.config.trx) {
          return _this2._runBatch(migrations, 'down', _this2.config.trx);
        }
        if (transactionForAll) {
          return _this2.knex.transaction(function (trx) {
            return _this2._runBatch(migrations, 'down', trx);
          });
        }
  ...
  Migrator.prototype.status = function status(config) {
    this.config = this.setConfig(config);
    var trx = this.config.trx || this.knex;

    return _bluebird2.default.all([trx(this.config.tableName).select('*'), this._listAll()]).spread(function (db, code) {
  ...
  Migrator.prototype.currentVersion = function currentVersion(config) {
    this.config = this.setConfig(config);
    return this._listCompleted(this.config.trx).then(function (completed) {
  ...
  Migrator.prototype.forceFreeMigrationsLock = function forceFreeMigrationsLock(config) {
    ...
    return trx.schema.hasTable(lockTable).then(function (exist) {
      return exist && _this3._freeLock(trx);
    });
  };
  ...
  Migrator.prototype._runBatch = function _runBatch(migrations, direction, trx) {
    ...
    .then(function () {
      ...
      if (direction === 'up') migrations = (0, _difference3.default)(migrations, completed);
    }).then(function () {
  ...
  Migrator.prototype._migrationData = function _migrationData(trx) {
    return _bluebird2.default.all([this._listAll(), this._listCompleted(trx)]);
  };
  ...
  Migrator.prototype._getLastBatch = function _getLastBatch() {
    var _config = this.config,
        ...
        _config$trx = _config.trx,
        trx = _config$trx === undefined ? this.knex : _config$trx;

    return trx(tableName).where('batch', function (qb) {
